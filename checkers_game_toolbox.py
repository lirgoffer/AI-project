...
class CheckerBoard:
            ...
            for iy in range(8):
                ...
                if (ix + iy) % 2 == 1 and (iy < 3 or iy > 4):
                    ...
                    if self.visual is None:
                        ...
                        new_checker = Checker(key=new_key, playerId=playerId_, sizes=self.visual.checker_sizes,
                                              colors=col_checker)
                ...
                if self.visual is None:
                    ...
                    new_square = Square(pos=(ix, iy), checker=new_checker, size=self.visual.square_size,
                                        coord_x=self.visual.coord_X[ix], coord_y=self.visual.coord_Y[iy],
                                        color=col_square)
    ...
    def __getPossibleSpots(self, checker):
        ...
        if checker.isQueen:
            Nbh = checker.square.getNeighborhood()
            pos_moves = [Nbh[str_pos] for str_pos in ['up-right', 'up-left', 'down-right', 'down-left'] if
                         Nbh[str_pos] is not None]
        ...
        occ_spots = [pos for pos in pos_moves if
                     pos not in free_spots and self.getSquare(*pos).checker.playerId != checker.playerId]
    ...
    def displayScore(self):
        ...
        if self.visual is not None:
            self.getSquare(0, 0).display(empty=True, text=str(len(self.checkers['1'])),
                                         colorText=self.visual.checker_col_1[0])
            self.getSquare(7, 7).display(empty=True, text=str(len(self.checkers['2'])),
                                         colorText=self.visual.checker_col_2[0])
    ...
    def playMCTSAgainstRandom(self, mcts_player, draw_after=None):
        ...
        while True:
            ...
            if not isMCTSTurn:
                ...
                isAlive = self.playRandomMove(3 - mcts_player.playerId,
                                              forcedTakeRule=mcts_player.__dict__['forcedTakeRule'])
                isMCTSTurn = True
                i_play += 1
