...
class MCTS:
    ...
    def expand(self):
        ...
        new_child = MCTS(playerId=next_playerId, boardgame=copy_board, parent=self,
                         ...
                         thinkingTime=self.thinkingTime, useTransfer=self.useTransfer,
                         forcedTakeRule=self.forcedTakeRule)
    ...
    def draw(self):
        if not self.isLeaf:
            str_node = self.depth * '          ' + '(' + (
                        str(self.visitCount) + ',' + str(self.reward / self.visitCount)[:5] + ')').ljust(3) + '----'
            ...
            if np.sum(self.boardgame.matrix != new_boardgame.matrix) == 0:
                ...
                if self.useTransfer:
                    ...
                    for node in self.nodes:
                        if np.sum(
                                node.boardgame.matrix != new_boardgame.matrix) == 0 and node.playerId == self.playerId:
    ...
    def optimistic_selection(self):
        ...
        if self.isFullyExpanded and not self.hasEnded:
            ...
            for i, child in enumerate(self.expanded_children):
                ...
                child_UCB = child.reward / child.visitCount + self.CONST_UCB * np.sqrt(
                    np.log(self.visitCount) / child.visitCount)
        ...
        if not self.hasEnded:
            ...
            while endClock - startClock < self.thinkingTime:
                ...
                if not selectedNode.hasEnded:
                    ...
                    reward, outcome = self.rewardDesign(bool_true_winner, selectedNode.nTurns, self.depth_RollOut,
                                                        delta)
